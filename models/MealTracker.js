...
async function trackMeal(UserID, MealID, DateTime, Amount) {
    const adjustedDatetime = moment.utc(DateTime).add(2, 'hours').format('YYYY-MM-DD HH:mm:ss');
    console.log('Meal logged:', { UserID, MealID, adjustedDatetime, Amount });
    
    await sql.connect(config);
    await sql.query`
        INSERT INTO Intake (UserID, MealID, DateTime, Amount)
        VALUES (${UserID}, ${MealID}, ${adjustedDatetime}, ${Amount})`;
